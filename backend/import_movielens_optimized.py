#!/usr/bin/env python
"""
Import MovieLens data tối ưu cho Collaborative Filtering và Demographic Filtering
Sử dụng các script hiện có và chỉ import những gì cần thiết
"""
import os
import sys
import django
...
def import_movielens_ratings_only():
    """Import chỉ ratings từ MovieLens - cần thiết cho CF"""
    print("📥 IMPORTING MOVIELENS RATINGS FOR CF")
    print("=" * 50)

    # Sử dụng script có sẵn
    try:
        call_command('import_movielens_reviews',
                    '--dataset-size', 'small',
                    '--generate-text', 'True')
        print("✅ MovieLens ratings imported successfully")
    except Exception as e:
        print(f"❌ Error importing ratings: {str(e)}")
        print("   Trying alternative method...")

        # Fallback: import thủ công
        import_ratings_manual()

def import_ratings_manual():
    """Import ratings thủ công nếu script không hoạt động"""
    print("🛠️ Manual ratings import...")
    ...
    print(f"✅ Manual import completed:")
...
def main():
    """Main function"""
    print("🚀 MOVIELENS OPTIMIZED IMPORT FOR CF AND DF")
    print("=" * 60)

    # 1. Import MovieLens ratings
    import_movielens_ratings_only()
