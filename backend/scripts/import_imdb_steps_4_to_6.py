#!/usr/bin/env python
"""
Script import tối ưu cho IMDB dataset từ bước 4-6
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'imdb_import_steps_4_6_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class OptimizedIMDBImporter:
    ...
    def import_steps_4_to_6(self):
        """Import steps 4-6 with optimization"""
        if not self._check_dataset_files():
            return False

        try:
            self.start_time = time.time()
            logger.info("Starting IMDB data import (Steps 4-6)...")
