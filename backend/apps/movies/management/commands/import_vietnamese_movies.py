#!/usr/bin/env python3
"""
Import Vietnamese movies from TMDB API
Fetches movies from Vietnam with both English and Vietnamese content
"""
...
class Command(BaseCommand):
    help = 'Import Vietnamese movies from TMDB API with bilingual support'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=1000,
            help='Maximum number of movies to import (default: 1000)'
        )
        parser.add_argument(
            '--batch-size',
            type=int,
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.0,
            help='Minimum rating to import (default: 0.0 - no minimum for Vietnamese movies)'
        )
        parser.add_argument(
            '--include-adult',
            action='store_true',
        ...
        parser.add_argument(
            ...
            type=str,
            default='VN',
            help='Region code for movie search (default: VN for Vietnam)'
        )
        parser.add_argument(
            '--language',
            type=str,
            default='vi-VN',
            help='Primary language for movie search (default: vi-VN)'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"🚀 Starting Vietnamese movie import..."))
        ...
        self.stdout.write(f"   - Min rating: {min_rating} (no minimum for Vietnamese movies)")
        ...
        self.stdout.write(f"   - Import criteria: All Vietnamese movies with titles (no rating/vote restrictions)")

        # Start import process
        try:
            imported_count = self.import_vietnamese_movies(
                api_key=api_key,
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ Import completed! Imported {imported_count} movies."))
    ...
    def import_vietnamese_movies(self, api_key, max_movies, batch_size, year_from, year_to,
                               min_rating, include_adult, dry_run, update_existing, region, language):
        """Main import function"""
        imported_count = 0
        skipped_count = 0
        updated_count = 0

        # Search for Vietnamese movies by year
        for year in range(year_from, year_to + 1):
            if imported_count >= max_movies:
                break

            self.stdout.write(f"🔍 Searching for movies from {year}...")

            # Get movies from TMDB for this year
            movies_data = self.get_vietnamese_movies_by_year(
                api_key, year, region, language, include_adult
            )

            if not movies_data:
                self.stdout.write(f"   No movies found for {year}")
                continue

            self.stdout.write(f"   Found {len(movies_data)} movies for {year}")
                ...
                for movie_data in batch:
                    ...
                    try:
                        ...
                        if not self.should_import_movie(movie_data, min_rating):
                            ...
                            # Only skip if no title
                            title = movie_data.get('title', 'Unknown')
                            self.stdout.write(f"   ⏭️ Skipped (no title): {title}")
                            skipped_count += 1
                            continue

                        # Import or update movie
                        result = self.process_movie(
                            movie_data, api_key, dry_run, update_existing
                        )

                        if result == 'imported':
                            imported_count += 1
                            self.stdout.write(f"   ✅ Imported: {movie_data.get('title', 'Unknown')}")
                        elif result == 'updated':
                            updated_count += 1
                            self.stdout.write(f"   🔄 Updated: {movie_data.get('title', 'Unknown')}")
                        elif result == 'skipped':
                            skipped_count += 1

                    except Exception as e:
                        self.stdout.write(f"   ❌ Error processing movie: {str(e)}")
                        logger.error(f"Error processing movie: {str(e)}", exc_info=True)
                        continue

                # Progress update
                self.stdout.write(f"   📊 Progress: {imported_count}/{max_movies} imported, {skipped_count} skipped")
    ...
    def get_vietnamese_movies_by_year(self, api_key, year, region, language, include_adult):
        """Get Vietnamese movies from TMDB for a specific year"""
        try:
    ...
    def filter_vietnamese_movies(self, movies):
        """Filter movies to find Vietnamese content - now simplified since we use with_original_language=vi"""
        # Since we're using with_original_language=vi, all movies are already Vietnamese
        return movies

    def is_vietnamese_movie(self, movie):
        """Check if a movie is Vietnamese - simplified since we use with_original_language=vi"""
        # With the new API approach, all movies returned are Vietnamese
        return True

    def should_import_movie(self, movie_data, min_rating):
        """Check if movie meets import criteria - simplified for Vietnamese movies"""
    ...
    def process_movie(self, movie_data, api_key, dry_run, update_existing):
        ...
        if existing_movie and not update_existing:
            self.stdout.write(f"   ⏭️ Skipped (already exists): {movie_data.get('title', 'Unknown')} (TMDB: {tmdb_id})")
            return 'skipped'
    ...
    def create_movie_data(self, movie_data, api_key):
        ...
        # Create movie with data from the API response
        movie = Movie(
            ...
            status='RELEASED',  # Most Vietnamese movies are released
        ...
        # Set titles in both languages - use original_title for Vietnamese
        if movie_data.get('original_language') == 'vi':
            movie.title_vi = movie_data.get('original_title', movie.title)
            movie.title_en = movie_data.get('title', movie.title)
        else:
            movie.title_en = movie_data.get('title', movie.title)
            movie.title_vi = movie_data.get('original_title', movie.title)

        # Set overviews from detailed data
        if detailed_data:
        ...
        # Create cast records if available
        if detailed_data and detailed_data.get('credits'):
            self.create_movie_cast(movie, detailed_data['credits'])
    ...
    def update_movie_data(self, movie, movie_data, api_key):
        ...
        if detailed_data:
            ...
            movie.title_vi = detailed_data.get('title', movie.title_vi)
            movie.overview_en = detailed_data.get('overview', movie.overview_en)
            movie.overview_vi = detailed_data.get('overview', movie.overview_vi)
    ...
    def get_movie_details(self, tmdb_id, api_key):
        ...
        try:
            ...
            # Get additional details like credits, videos, images
            credits_data = TMDBService._make_request(
                f"/movie/{tmdb_id}/credits",
                params={"language": "en-US"},
                use_cache=False
            )

            videos_data = TMDBService._make_request(
            ...
            if credits_data:
                combined_data['credits'] = credits_data

            # Add videos information
            if videos_data:
                combined_data['videos'] = videos_data
    ...
    def create_movie_cast(self, movie, credits_data):
        """Create movie cast records"""
        try:
            from apps.movies.models import MovieCast

            # Add cast (actors)
            cast_list = credits_data.get('cast', [])
            for i, cast_member in enumerate(cast_list[:10]):  # Limit to top 10 cast
                MovieCast.objects.create(
                    movie=movie,
                    name=cast_member.get('name', ''),
                    role='ACTOR',
                    main_character=cast_member.get('character', ''),
                    order=cast_member.get('order', i),
                    tmdb_id=cast_member.get('id'),
                    profile_path=cast_member.get('profile_path'),
                    popularity=cast_member.get('popularity')
                )

            # Add crew (directors, writers, etc.)
            crew_list = credits_data.get('crew', [])
            for crew_member in crew_list[:5]:  # Limit to top 5 crew
                ...
                if 'director' in job:
                    ...
                    role = 'PRODUCER'
                elif 'cinematographer' in job or 'camera' in job:
                    role = 'CINEMATOGRAPHER'
                elif 'editor' in job:
                    role = 'EDITOR'
                elif 'composer' in job or 'music' in job:
                    role = 'COMPOSER'
            ...
            logger.error(f"Error creating cast for movie {movie.id}: {str(e)}")
